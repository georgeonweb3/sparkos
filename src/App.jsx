...
function Settings({ apiKey, onSave }) {
  ...
  return (
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      <Card style={{ background: `linear-gradient(135deg, ${COLORS.accent}22, ${COLORS.card})` }}>
        <div style={{ fontSize: 13, color: COLORS.accentGlow, fontWeight: 700, marginBottom: 4 }}>
          OpenRouter API Key
        </div>
        <div style={{ fontSize: 12, color: COLORS.muted, lineHeight: 1.6 }}>
          Required for Bug Scout AI analysis. Get a free key at openrouter.ai — you already have one from your Spark bot setup.
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <GlowDot color={apiKey ? COLORS.green : COLORS.red} size={8} pulse={!apiKey} />
          <span style={{ fontSize: 13, color: apiKey ? COLORS.green : COLORS.red }}>
            {apiKey ? "Bug Scout ready" : "No API key — Bug Scout disabled"}
          </span>
        </div>
      </Card>
      ...
      <Card>
        ...
        <a href="https://openrouter.ai/keys" target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
          <div style={{
            padding: "10px 14px",
            background: COLORS.surface,
            border: `1px solid ${COLORS.accentGlow}`,
...
export default function SparkOS() {
  ...
  const [apiKey, setApiKey] = useState(() => {
    try { return localStorage.getItem("sparkos_or_key") || ""; } catch { return ""; }
  });

  const saveKey = (k) => {
    setApiKey(k);
    try { localStorage.setItem("sparkos_or_key", k); } catch {}
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ paddingBottom: 32 }}>
        ...
        {activeTab === "scout" && <BugScout apiKey={apiKey} onNeedKey={() => setActiveTab("settings")} />}
        {activeTab === "onboard" && <Onboarding />}
        {activeTab === "settings" && <Settings apiKey={apiKey} onSave={saveKey} />}
      </div>
    </div>
  );
}
